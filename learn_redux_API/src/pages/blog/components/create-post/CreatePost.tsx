  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (editingPost) {
      dispatch(
        ...
        .catch((err) => {
          setError(err.error.publishDate)
        })
    } else {
      try {
        const { id, ...res } = formData
        const submitFormData = { ...res }
        await dispatch(addPost(submitFormData)).unwrap()
        setFormData(initialPost)
        if (error) {
          setError('')
        }
  ...
  return (
    <div>
      <form onSubmit={handleSubmit} onReset={handleCancelEditingPost}>
        <div className='mb-6'>
          <label
            // onClick={() => dispatch({ type: 'blog/startttt' })}
            htmlFor='title'
        ...
        <div className='mb-6'>
          <label
            htmlFor='publishDate'
            className={`mb-2 block text-sm font-medium ${error ? 'text-red-500' : 'text-gray-900'} `}
          >
            Publish Date
          </label>
          <input
            type='datetime-local'
            id='publishDate'
            className={`block w-56 rounded-lg border p-2.5 text-sm focus:outline-none ${error ? 'border-red-300 bg-red-50 text-red-900 focus:border-red-500  focus:ring-red-500' : 'border-gray-300 bg-gray-50 text-gray-900 focus:border-blue-500  focus:ring-blue-500'} `}
            placeholder='Title'
            required
